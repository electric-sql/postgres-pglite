...
docker run --rm \
  --user 1000:1000 \
  --env PGLITE_POSTGRES_TEST_TARGET="${PGLITE_POSTGRES_TEST_TARGET:-check}" \
  --env PGLITE_POSTGRES_TEST_JOBS="${PGLITE_POSTGRES_TEST_JOBS:-2}" \
  ...
  bash -lc '
    ...
    ./postgres-pglite/pglite/multi-memory/tests/test-postgres.sh /work
  '
