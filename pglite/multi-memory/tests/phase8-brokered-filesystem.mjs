...
if (!output) {
  throw new Error(
    'usage: phase8-brokered-filesystem.mjs REPO_ROOT WASM GLUE DATA OUTPUT',
  )
}
...
try {
    ...
    `${JSON.stringify(
      {
        schema: 1,
        status: 'pass',
        contract: 'existing-noncloneable-pglite-base-filesystem',
        strategy: 'supervisor-sab-broker',
  ...
  console.log('Phase 8 brokered filesystem gate: PASS')
} finally {
  await Promise.allSettled(sessions.map((session) => session.close()))
