...
assert.ok(
  ['two-domain-generic', 'two-domain-generic-private-fast-path'].includes(
    report.abi.profile,
  ),
)
for (const key of [
...
const transformed = (await instantiateTransformed(privateMemory, globalMemory))
  .instance.exports
...
for (const bad of [0, 0xc0000001 | 0]) {
  assert.throws(
    () => transformed.scalar_i32_load(bad),
...
assert.deepEqual(
  [...new Uint8Array(globalMemory.buffer, 400, 16)],
  [...Array(16).keys()],
)
transformed.bulk_copy(500, tagGlobal(400), 16)
assert.deepEqual(
  [...new Uint8Array(privateMemory.buffer, 500, 16)],
  [...Array(16).keys()],
)
transformed.bulk_fill(tagGlobal(520), 0xa5, 16)
assert.deepEqual(
  [...new Uint8Array(globalMemory.buffer, 520, 16)],
  Array(16).fill(0xa5),
)

const aliased = makeMemory()
const aliasExports = (await instantiateTransformed(aliased, aliased)).instance
  .exports
...
const fuzz = (await instantiateTransformed(fuzzMemory, fuzzMemory)).instance
  .exports
