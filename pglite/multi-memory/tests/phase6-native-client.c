...
{
	PGconn	   *target = connect_client(conninfo, "phase6-native-cancel");
	PGcancelConn *cancel;
	PGresult   *result;
...
test_copy_and_backpressure(PGconn *conn)
{
	...
	exec_command(conn, "DROP TABLE IF EXISTS phase6_native_copy");
	exec_command(conn,
				 "CREATE TABLE phase6_native_copy(id int PRIMARY KEY, payload text)");

	result = PQexec(conn, "COPY phase6_native_copy FROM STDIN");
	...
	result = PQexec(conn,
				"COPY (SELECT payload FROM phase6_native_copy ORDER BY id) TO STDOUT");
...
test_concurrent_progress(const char *conninfo)
{
	...
	for (int index = 0; index < 8; index++)
	{
		...
		snprintf(application_name, sizeof(application_name),
				 "phase6-native-concurrent-%d", index);
...
{
	PGconn	   *client = connect_client(conninfo, "phase6-native-disconnect");
	PGresult   *result;
	...
	for (int attempt = 0; attempt < 100; attempt++)
	{
		result = PQexec(control,
					...
					"WHERE application_name = 'phase6-native-disconnect'");
...
main(int argc, char **argv)
{
	...
	conninfo = argv[1];
	control = connect_client(conninfo, "phase6-native-control");
	result = PQexec(control,
	...
	if (strcmp(PQgetvalue(result, 0, 0), "postgres") != 0 ||
		...
		strcmp(PQgetvalue(result, 0, 2), "phase6-native-control") != 0)
	{
	...
	PQfinish(control);
	puts("Phase 6 native libpq cancel/COPY/backpressure gate: PASS");
	return 0;
}
