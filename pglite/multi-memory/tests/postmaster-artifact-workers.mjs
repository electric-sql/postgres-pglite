...
if (!outputPath) {
  throw new Error(
    'usage: postmaster-artifact-workers.mjs WASM GLUE DATA OUTPUT',
  )
}

const directory = await mkdtemp(join(tmpdir(), 'pglite-postmaster-nodefs-'))
...
const workerPath = new URL('./postmaster-artifact-worker.mjs', import.meta.url)
...
try {
  ...
  console.log('Transformed postmaster Worker test: PASS')
} finally {
