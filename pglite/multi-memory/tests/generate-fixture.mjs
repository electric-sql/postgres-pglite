...
const scalarLoads = [
  ...
  ['i32.load8_s', 'i32'], ['i32.load8_u', 'i32'],
  ['i32.load16_s', 'i32'], ['i32.load16_u', 'i32'],
  ['i64.load8_s', 'i64'], ['i64.load8_u', 'i64'],
  ['i64.load16_s', 'i64'], ['i64.load16_u', 'i64'],
  ['i64.load32_s', 'i64'], ['i64.load32_u', 'i64'],
]
for (const [op, result] of scalarLoads) {
  fn(`scalar_${op.replaceAll('.', '_')}`, ['i32'], result,
    `(${op} offset=7 align=1 (local.get $p0))`, 'load')
}
...
for (const [op, type] of scalarStores) {
  fn(`scalar_${op.replaceAll('.', '_')}`, ['i32', type], '',
    `(${op} offset=7 align=1 (local.get $p0) (local.get $p1))`, 'store')
}

const atomicShapes = [
  ['i32', '', 'i32'], ['i64', '', 'i64'],
  ['i32', '8', 'i32'], ['i32', '16', 'i32'],
  ['i64', '8', 'i64'], ['i64', '16', 'i64'], ['i64', '32', 'i64'],
]
for (const [base, width, type] of atomicShapes) {
  ...
  fn(`atomic_${load.replaceAll('.', '_')}`, ['i32'], type,
    `(${load} offset=${align} (local.get $p0))`, 'atomic-load')
  fn(`atomic_${store.replaceAll('.', '_')}`, ['i32', type], '',
    `(${store} offset=${align} (local.get $p0) (local.get $p1))`, 'atomic-store')
  for (const operation of ['add', 'sub', 'and', 'or', 'xor', 'xchg']) {
    const op = `${base}.atomic.rmw${width}.${operation}${width ? '_u' : ''}`
    fn(`atomic_${op.replaceAll('.', '_')}`, ['i32', type], type,
      `(${op} offset=${align} (local.get $p0) (local.get $p1))`, 'atomic-rmw')
  }
  const cmpxchg = `${base}.atomic.rmw${width}.cmpxchg${width ? '_u' : ''}`
  fn(`atomic_${cmpxchg.replaceAll('.', '_')}`, ['i32', type, type], type,
    `(${cmpxchg} offset=${align} (local.get $p0) (local.get $p1) (local.get $p2))`,
    'atomic-cmpxchg')
}

fn('atomic_wait32', ['i32', 'i32', 'i64'], 'i32',
  '(memory.atomic.wait32 offset=4 (local.get $p0) (local.get $p1) (local.get $p2))',
  'atomic-wait')
fn('atomic_wait64', ['i32', 'i64', 'i64'], 'i32',
  '(memory.atomic.wait64 offset=8 (local.get $p0) (local.get $p1) (local.get $p2))',
  'atomic-wait')
fn('atomic_notify', ['i32', 'i32'], 'i32',
...
for (const op of simdLoads) {
  fn(`simd_${op.replaceAll('.', '_')}`, ['i32'], 'i32',
    `(i32x4.extract_lane 0 (${op} offset=3 align=1 (local.get $p0)))`,
    op === 'v128.load' ? 'load' : 'simd-load')
}
for (const [bits, lane] of [[8, 15], [16, 7], [32, 3], [64, 1]]) {
  fn(`simd_load${bits}_lane`, ['i32', 'i32'], 'i32',
    `(i32x4.extract_lane 0 (v128.load${bits}_lane offset=3 align=1 ${lane} (local.get $p0) (i32x4.splat (local.get $p1))))`,
    'simd-lane-load')
  fn(`simd_store${bits}_lane`, ['i32', 'i32'], '',
    `(v128.store${bits}_lane offset=3 align=1 ${lane} (local.get $p0) (i32x4.splat (local.get $p1)))`,
    'simd-lane-store')
}
fn('simd_v128_store', ['i32', 'i32'], '',
  '(v128.store offset=3 align=1 (local.get $p0) (i32x4.splat (local.get $p1)))', 'store')

fn('bulk_copy', ['i32', 'i32', 'i32'], '',
  '(memory.copy (local.get $p0) (local.get $p1) (local.get $p2))', 'memory-copy')
fn('bulk_fill', ['i32', 'i32', 'i32'], '',
  '(memory.fill (local.get $p0) (local.get $p1) (local.get $p2))', 'memory-fill')
fn('bulk_init', ['i32', 'i32', 'i32'], '',
  '(memory.init $blob (local.get $p0) (local.get $p1) (local.get $p2))', 'memory-init')
fn('bulk_drop', [], '', '(data.drop $blob)', 'data-drop')
fn('memory_size', [], 'i32', '(memory.size)', 'memory-size')
fn('memory_grow', ['i32'], 'i32', '(memory.grow (local.get $p0))', 'memory-grow')
fn('atomic_fence', [], '', '(atomic.fence)', 'atomic-fence')

fn('side_effect_load', ['i32'], 'i32',
  '(i32.load (call $address_with_side_effect (local.get $p0)))', 'load')
fn('side_effect_store', ['i32', 'i32'], '',
  '(i32.store (call $address_with_side_effect (local.get $p0)) (local.get $p1))', 'store')
fn('side_effect_copy', ['i32', 'i32', 'i32'], '',
