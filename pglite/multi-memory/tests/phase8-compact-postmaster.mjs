...
if (!output) {
  throw new Error(
    'usage: phase8-compact-postmaster.mjs REPO_ROOT WASM GLUE DATA DEDICATED_JSON OUTPUT',
  )
}

const dedicated = JSON.parse(await readFile(dedicatedPath, 'utf8'))
const privateInitialBytes = 32 * 1024 * 1024
const dataDirectory = await mkdtemp(join(tmpdir(), 'pglite-phase8-compact-'))
const sessions = []
let postmaster
...
try {
  ...
  await control.exec(`
    CREATE UNLOGGED TABLE phase8_compact(value int NOT NULL);
    INSERT INTO phase8_compact SELECT generate_series(1, 250000);
    ALTER TABLE phase8_compact SET (parallel_workers = 2);
    ANALYZE phase8_compact;
  `)
  await leader.exec(`
  ...
  const explained = await leader.query(`
    ...
    SELECT sum(value)::bigint FROM phase8_compact
  `)
  assert.match(
  ...
  assert.deepEqual(
    (
      await leader.query(
        'SELECT sum(value)::bigint AS total FROM phase8_compact',
      )
    ).rows,
  ...
  console.log('Phase 8 compact postmaster and memory-value gate: PASS')
} finally {
