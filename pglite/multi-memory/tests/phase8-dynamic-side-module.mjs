...
if (!output) {
  throw new Error(
    'usage: phase8-dynamic-side-module.mjs REPO_ROOT WASM GLUE DATA RAW_MODULE TRANSFORMED_MODULE AUDIT OUTPUT',
  )
}

const audit = JSON.parse(await readFile(auditPath, 'utf8'))
assert.equal(audit.status, 'pass')
const dataDirectory = await mkdtemp(join(tmpdir(), 'pglite-phase8-dylink-'))
...
try {
    ...
    `${JSON.stringify(
      {
        schema: 1,
        status: 'pass',
        profile: 'transformed-dynamic-side-module',
        rejectedUntransformedModule: true,
        rejectedIncompatibleABI: true,
        dylinkRelocations: true,
        privateStaticIsolation: true,
        globalMemoryDereference: true,
        scopedMemoryDereference: true,
        firstCall,
        secondCall,
        otherBackend,
        before,
  ...
  console.log('Phase 8 transformed dynamic side-module runtime gate: PASS')
} finally {
