...
const postgres = await createPostgres({
  ...
  preRun: [
    (mod) => {
      mod.FS.mkdir('/postmaster-nodefs')
      mod.FS.mount(
...
parentPort.on('message', (message) => {
  if (message.type === 'write') {
    postgres.FS.writeFile('/postmaster-nodefs/parameter-file', message.value)
    parentPort.postMessage({ type: 'wrote' })
  } else if (message.type === 'read') {
    parentPort.postMessage({
      type: 'read',
      value: postgres.FS.readFile('/postmaster-nodefs/parameter-file', {
        encoding: 'utf8',
      }),
    })
