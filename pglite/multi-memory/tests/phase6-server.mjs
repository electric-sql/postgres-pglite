...
if (!portText) {
  throw new Error(
    'usage: phase6-server.mjs REPO_ROOT WASM GLUE DATA NATIVE TESTLIB TEST_OUTPUT PGDATA READY RESULT PORT',
  )
}
...
const parallelParams =
  process.env.PGLITE_PHASE6_ENABLE_PARALLEL === 'true'
    ? [
        '-c',
...
async function createSessionWhenReady(server) {
  ...
  throw lastError ?? new Error('Phase 6 postmaster did not become ready')
}
