...
node22 "${MM_ROOT}/tests/phase4-audit.mjs" \
  "${INPUT}" "${CANDIDATE}" "${REPORT}" "${OUT}/artifact-audit.json"
node22 "${MM_ROOT}/tests/phase4-artifact-workers.mjs" \
  "${CANDIDATE}" "${GLUE}" "${DATA}" "${OUT}/worker-gate.json"

pnpm -C "${REPO_ROOT}/packages/pglite" typecheck
pnpm -C "${REPO_ROOT}/packages/pglite" build >/tmp/pglite-phase4-build.log
pnpm -C "${REPO_ROOT}/packages/pglite" exec vitest run \
  tests/postmaster-phase4.test.ts --maxWorkers=1 --minWorkers=1
pnpm -C "${REPO_ROOT}/packages/pglite" exec eslint \
  src/postmaster tests/postmaster-phase4.test.ts --max-warnings=0
...
node22 - \
  "${OUT}/artifact-audit.json" "${OUT}/worker-gate.json" \
  "${OUT}/summary.json" <<'NODE'
const fs = require('node:fs')
const [auditPath, workerPath, output] = process.argv.slice(2)
const audit = JSON.parse(fs.readFileSync(auditPath, 'utf8'))
const workers = JSON.parse(fs.readFileSync(workerPath, 'utf8'))
fs.writeFileSync(output, `${JSON.stringify({
  ...
  workers,
  execBackend: true,
  temporaryBackendParameters: 'nodefs',
  controlSAB: true,
  queuedSignals: true,
  supervisorTimers: true,
  sharedWordSemaphores: true,
  virtualSockets: true,
}, null, 2)}\n`)
NODE

echo 'Phase 4 process portability gate: PASS'
