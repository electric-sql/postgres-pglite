...
const [repoRoot, wasm, glue, data, nativeRoot, output] = process.argv.slice(2)
if (!output) {
  throw new Error(
    'usage: phase5-socket.mjs REPO_ROOT WASM GLUE DATA NATIVE_ROOT OUTPUT',
  )
}
...
async function main() {
  ...
  const root = await mkdtemp(join(tmpdir(), 'pglite-phase5-socket-'))
  ...
  try {
      ...
      `${JSON.stringify(
        {
          ...
          tcp: tcpAddress,
          unix: unixAddress,
          readiness: readiness.stdout.trim(),
          tcpResult: tcpResult.stdout.trim().split('\n'),
          unixResult: unixResult.stdout.trim().split('\n'),
          tools: versions.map(({ stdout }) => stdout.trim()),
          beforeShutdown,
          shutdown,
        },
        null,
    ...
    console.log('Phase 5 native TCP/Unix socket gate: PASS')
  } finally {
