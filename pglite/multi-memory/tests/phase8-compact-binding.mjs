...
if (!outputPath) {
  throw new Error('usage: phase8-compact-binding.mjs WASM GLUE DATA OUTPUT')
}
...
const dedicatedEquivalentBytes =
  dedicatedPrivate.buffer.byteLength + dedicatedScoped.buffer.byteLength
const compactEquivalentBytes = rootPrivate.buffer.byteLength
  ...
  `${JSON.stringify(
    {
      schema: 1,
      status: 'pass',
      profile: 'compact-memory-0-memory-2-binding',
      registryOffset,
      frontierAfterRegistry,
      frontierAfterScoped,
      rootMemoryBytes: rootPrivate.buffer.byteLength,
      dedicatedEquivalentBytes,
      compactEquivalentBytes,
      compactMinusDedicatedBytes:
        compactEquivalentBytes - dedicatedEquivalentBytes,
      privateAllocatorNonOverlap: true,
      inheritedWorkerBinding: true,
      unifiedAtomicFrontier: true,
      corruptionContainment: 'allocator-safe-but-no-linear-memory-isolation',
    },
    null,
    2,
  )}\n`,
...
child.dispose()
dedicated.dispose()
console.log('Phase 8 compact memory binding artifact gate: PASS')
