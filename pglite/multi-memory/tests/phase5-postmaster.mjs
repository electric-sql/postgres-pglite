...
if (!output) {
  throw new Error(
    'usage: phase5-postmaster.mjs REPO_ROOT WASM GLUE DATA OUTPUT',
  )
}
...
async function main() {
  ...
  const dataDirectory = await mkdtemp(
    join(tmpdir(), 'pglite-phase5-postmaster-'),
  )
  let postmaster
  ...
  try {
    postmaster = await withTimeout(
      PGlitePostmaster.create({
        ...
        debug: process.env.PGLITE_PHASE5_DEBUG === '1',
      }),
      60_000,
      'postmaster startup',
    )
    console.log('phase5: postmaster started')

    const before = postmaster.diagnostics()
    const { connection, reader } = await connectWhenReady(postmaster)
    console.log('phase5: protocol backend ready')
    ...
    const ddl = await simpleQuery(
      reader,
      connection,
      'CREATE TABLE phase5_gate(id int primary key, value text);' +
        "INSERT INTO phase5_gate VALUES (42, 'multi-memory');" +
        'SELECT id, value FROM phase5_gate;',
    )
    assert.deepEqual(ddl.rows, [['42', 'multi-memory']])
    console.log('phase5: protocol DDL/DML passed')
    ...
    console.log('phase5: normal sessions ready')
    ...
    await sessionA.exec(
      'CREATE TEMP TABLE phase5_private(value text);' +
        "INSERT INTO phase5_private VALUES ('session-a');",
    )
    const isolated = await sessionB.query(
      "SELECT to_regclass('pg_temp.phase5_private') IS NULL AS isolated",
    )
    ...
    await sessionA.listen('phase5_notify', (payload) =>
      resolveNotification(payload),
    )
    await sessionB.exec("NOTIFY phase5_notify, 'multi-memory'")
    assert.equal(
      await withTimeout(notification, 15_000, 'LISTEN/NOTIFY delivery'),
      'multi-memory',
    )
    console.log('phase5: concurrent sessions and notifications passed')
      ...
      `${JSON.stringify(
        {
          schema: 1,
          status: 'pass',
          startup: startupFrames.map(({ type }) => type),
          selectRows: select.rows,
          ddlRows: ddl.rows,
          normalInterfaceRows: normalQuery.rows,
          concurrentRows: concurrent.map((result) => result.rows),
          before,
          after,
          shutdown,
          memory2: 'aliases-memory0',
          parallelQuery: 'disabled',
        },
        null,
        2,
      )}\n`,
    )
    console.log('Phase 5 postmaster/backend protocol gate: PASS')
  } finally {
...
async function connectWhenReady(postmaster) {
  ...
  for (let attempt = 0; Date.now() < deadline; attempt++) {
    ...
    if (attempt % 5 === 4)
      console.log(
        `phase5: backend not ready after ${attempt + 1} attempts: ${lastError}`,
      )
