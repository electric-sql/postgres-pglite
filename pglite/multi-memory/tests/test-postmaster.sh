...
if [[ "${PGLITE_POSTMASTER_TEST_REUSE_ARTIFACT:-false}" != true ]]; then
  ...
  PGLITE_POSTMASTER_BUILD_SOURCE_OUT="${SOURCE_OUT}" \
    "${MM_ROOT}/tests/build-postmaster.sh" "${REPO_ROOT}"
else
...
cc -O2 -Wall -Wextra -Werror \
  ...
  "${MM_ROOT}/tests/native-client-test.c" \
...
node22 "${MM_ROOT}/tests/filesystem-factory.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
  "${OUT}/filesystem-factory.json"

node22 "${MM_ROOT}/tests/postmaster-session.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
  "${OUT}/postmaster-session.json"

node22 "${MM_ROOT}/tests/socket-frontend.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
  "${NATIVE}" \
  "${OUT}/socket-frontend.json"
...
node22 "${MM_ROOT}/tests/postmaster-correctness.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
...
node22 "${MM_ROOT}/tests/compact-postmaster.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
...
node22 "${MM_ROOT}/tests/dynamic-side-module.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
...
node22 "${MM_ROOT}/tests/brokered-filesystem.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
...
node22 "${MM_ROOT}/tests/regression-server.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
...
fs.writeFileSync(output, `${JSON.stringify({
  schema: 1,
  status: 'pass',
  profile: 'postmaster-full-upstream-concurrency',
...
node22 "${MM_ROOT}/tests/postmaster-stress.test.mjs" \
  "${REPO_ROOT}" \
  "${ARTIFACT_OUT}/postmaster.wasm" \
  "${SOURCE_OUT}/bin/pglite.js" \
  "${SOURCE_OUT}/bin/pglite.data" \
