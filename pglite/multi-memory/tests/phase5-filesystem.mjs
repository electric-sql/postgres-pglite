...
if (!output) {
  throw new Error(
    'usage: phase5-filesystem.mjs REPO_ROOT WASM GLUE DATA OUTPUT',
  )
}

const dataDirectory = await mkdtemp(join(tmpdir(), 'pglite-phase5-filesystem-'))
let postmaster
let session
...
try {
  ...
  postmaster = await PGlitePostmaster.create({
    ...
    workerFilesystem: {
      module: join(repoRoot, 'packages/pglite/tests/worker-nodefs-factory.mjs'),
  ...
  console.log('Phase 5 pluggable Worker filesystem gate: PASS')
} finally {
